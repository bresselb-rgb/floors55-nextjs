...
export default function ProposalsManager({ proId }) {
  ...
  const handleEditChange = (field, value, isService = false) => {
      ...
      const newRetail = newWholesale * (1 + (updated.totals.margin / 100));
          ...
          quotes.map((quote) => (
            <div key={quote.id} className="flex flex-col md:flex-row md:items-center justify-between p-5 border border-gray-100 rounded-xl bg-gray-50 hover:bg-white hover:shadow-md transition-all gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button onClick={() => setEditingQuote(quote)} className="bg-white border border-gray-200 hover:border-gray-300 text-gray-500 px-3 py-2.5 rounded-lg transition-colors cursor-pointer outline-none" title="Edit Proposal">
                    ✏️
                </button>
