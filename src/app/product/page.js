...
export default function ProductPage() {
  ...
  // Track URL parameters independently
  const [urlColorSku, setUrlColorSku] = useState(null);
  ...
  // Sync hash routing specifically for the Product ID and Color
  useEffect(() => {
    const handleNavigation = () => {
      if (typeof window === 'undefined') return;
      
      // Catch and repair any double-hash corruptions from the back button
      const hashParts = window.location.hash.split('#');
      const hash = hashParts[hashParts.length - 1] || ''; // get the true last hash segment
      
      const possibleId = hash.split('?')[0];
      if (possibleId && possibleId !== productId) {
    ...
    // Listen to both hashchange and native popstate (Back Button)
  ...
  }, [user, productId, isAuthReady]);

  // Synchronize the visual active color with the data + URL
  useEffect(() => {
     if (productData && productData.colors) {
         if (urlColorSku) {
             ...
             if (found) {
                 ...
                 // Only set MAIN view if we just swapped to a new color from the URL
  ...
  const getMediaPath = (view) => {
    if (!productData || !activeColor) return null; // Fixed: return null instead of empty string
  ...
  const shareProduct = () => {
      const url = `${window.location.origin}${window.location.pathname}#${productId}?color=${activeColor?.sku || ''}`;
      if (navigator.share) {
            ...
            {productData.views && (
                <div className="mt-4 flex gap-3">
                    {productData.views.map(v => (
                         <img 
                            key={v}
                            src={v === 'VIDEO' ? 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23c5a059" width="48px" height="48px"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>' : (getMediaPath(v) || '/images/tbd.jpg')}
          ...
          <div className="flex-1 min-w-[320px]">
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(85px,1fr))] gap-3 my-6">
                {productData.colors?.map(c => {
                    ...
                    const lowerPath = `/images/${folderName}/${productData.imgPrefix || ''}${c.sku}_${swatchType}.jpg`.toLowerCase();
                    
                    return (
                        <div key={c.sku} className="cursor-pointer text-center group" onClick={() => { 
                            // Safely use Next.js router.replace to avoid history stack corruption
                            router.replace(`/product#${productId}?color=${c.sku}`, { scroll: false });
                            setUrlColorSku(c.sku);
                            setActiveColor(c);
                        }}>
                            <img src={lowerPath} onError={(e) => e.target.src = '/images/tbd.jpg'} className={`w-full aspect-square object-cover border-2 rounded-md transition duration-200 bg-gray-100 ${activeColor?.sku === c.sku ? 'border-gold shadow-[0_0_8px_rgba(197,160,89,0.4)]' : 'border-transparent group-hover:border-gray-300'}`} alt={c.name} />
